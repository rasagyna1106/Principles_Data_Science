...
Data include **hand grip strength** from a dynamometer and **participant characteristic** data.
...
This study explores the **relationship between grip strength and frailty** among female participants. Exploratory data analysis (EDA), visualization, and predictive modeling are applied to identify frailty determinants.
...
Open **`01_frailty_workflow.ipynb`** and run all cells.
...
All plots are exported as PNGs (e.g. in **`reports/figures/`** or Result folder). Each figure: 800×600 px, 300 DPI, with title, labeled axes, and legend where applicable.

### V1: Gender split for Math vs Reading (boxplots)

From the box plots, the performance of male and female students is shown for mathematics and reading scores. Female students perform slightly higher in **reading**, with a small lead in **mathematics** scores by male students. There is overlapping of scores between the two groups, indicating that performance is not sharply divided. Some scores act as **outliers** in the performance of these groups.
...
A bar chart shows performance by lunch type. **Standard lunch** students’ performance is higher than **free/reduced lunch** students. This suggests economic influences may affect student performance. Free/reduced lunch students may receive less support, hence lower scores; standard lunch students perform better in academics.

### V4: Math, Reading, Writing correlation (heatmap)

The heatmap shows **positive correlations** between math, reading, and writing. Reading and writing are most strongly correlated; students who perform well in reading tend to perform well in writing. Math is well correlated with both reading and writing. The heatmap suggests that skills in one subject can help improve performance in others.
...
1. Place **`StudentsPerformance.csv`** in the project root (download from assignment Box link if needed).
